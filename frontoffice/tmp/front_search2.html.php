...
class ShowImages
{
    ...
    public function showAllByTag($string)
    {
        ...
        foreach($string as $s){
            $ress2 = $con->query("SELECT COUNT(*) FROM `photos` LEFT JOIN `category` ON category.`c_id` = photos.`category` WHERE category.`c_visibility` = '1' AND photos.`p_visibility` = '1' AND ( photos.`p_id` LIKE '%".$s."%' OR photos.`tag` LIKE '%".$s."%' OR photos.`author` LIKE '%".$s."%' OR category.`category` LIKE '%".$s."%' OR photos.`show_place` LIKE '%".$s."%' OR photos.`show_data` LIKE '%".$s."%' ) ORDER BY photos.`p_id` ".$order." ");/*zwraca false jesli tablica nie istnieje*/
            $ress2 = $ress2->fetch(PDO::FETCH_ASSOC);
            $ress3[] = $ress2['COUNT(*)'];
        }
        //return $ress;
        //return array($ress0, $ress3, $string);
        return array($ress1);
    }
...
//var_dump($success[2]);
$success = $success[0];
//echo count($success->fetch(PDO::FETCH_ASSOC));
///////////////////////////
//var_dump($_COOKIE);
